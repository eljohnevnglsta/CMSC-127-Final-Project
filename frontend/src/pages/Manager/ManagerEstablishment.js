...
function ManagerEstablishment() {
  ...
  //fetch the establishment details immediately identified by its name
  useEffect(() => {
  ...
  return (
    <div className="my-10 overflow-hidden mx-20 min-h-screen round shadow-lg ">
      ...
      <div className="py-10 px-20 ">
        ...
        {updateWindow ? (
          <UpdateEstablishment show={setUpdateWindow} name={name} />
        ) : null}
        {addWindow ? (
          <AddFood
            show={setAddWindow}
        ...
        <div className="text-sm font-medium text-center text-gray-500 border-b border-gray-200 dark:text-gray-400 dark:border-gray-700 my-7 pl-8">
          <div className="flex flex-wrap -mb-px">
            <button
              className="inline-block p-4 font-semibold text-large border-b-2 border-transparent rounded-t-lg hover:border-b-sky-950 hover:text-gray-600 text-sky-950 px-6"
              onClick={() => {
                setShowReview(false);
              }}
            >
        ...
        {showReview ? (
          <EstablishmentReviewManager name={name} />
        ) : (
          <FoodListManager username={username} name={name} />
        )}
      </div>
    </div>
  );
