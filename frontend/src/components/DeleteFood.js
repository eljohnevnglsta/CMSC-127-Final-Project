...
function DeleteFood(props) {
  ...
  //calls the delete food backend feature by passing the required parameters
  function handleDelete() {
    fetch("http://localhost:3001/food-item/delete", {
      ...
      .then(() => {
        ...
        if (refresh && order) {
          refresh(order);
        } else navigate(`/manager/food-establishment/${name}`);
      });
  }
        ...
        <div className="flex justify-between items-center mb-4">
          <div className="w-11/12 self-center">
            ...
            <p className=" text-base mb-2 text-sky-950">
              Deleting this food will also delete all reviews associated with
              it.
            </p>
            <button
